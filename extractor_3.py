...
from calculate_translatability import lemmatize_translations, lemmatize_TECs, normalize_dictionary, score_translatability, normalize_TECs, generate_test_dictionary
...
def split_compounds(candidates):
	...
	flat_list = [[item[0], item[1], item[2]] for sublist in compounds for item in sublist if item[0] > 0.2]
...
def extract_compounds(candidates):
	...
	for compound in candidates:
		if spell[compound[1]] and spell[compound[2]] or duden_search.search(compound[1]) and duden_search.search(compound[2]):
			print("Assessing '{compound1}' - '{compound2}'...".format(compound1=compound[1], compound2=compound[2]))
	...
	compounds_dictionary = dict(zip([''.join([segments[0], segments[1].lower()]) for segments in compounds_final], compounds_final_translations))
	print('\n\n\nExtracted compounds:')
	...
	for word in dictionary:
		...
		with open(dictionary[word]['segments'], 'r', encoding="utf-8") as f:
			...
			if len(lines) == 0:
				...
				print("No segments found for '{word}.' This word has been removed from the candidate list.".format(word=word))
				...
				print("Possible TECs for '{word}'. Only one segment found.".format(word=word))
				print(TEC_list)
				print('\n\n')
