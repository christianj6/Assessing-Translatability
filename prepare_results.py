...
def export_data_full(dictionary, directory):
	...
	for item in dictionary:
		score, word, translations, TECs = dictionary[item]['score'], item, dictionary[item]['translations'], dictionary[item]['TECs']
...
def export_marked_text(text, neat_list, directory):
	...
	for tpl in neat_list:
		text_joined = text_joined.replace(tpl[1], '[{word}:{score}]'.format(word=tpl[1], score="{0:.2f}".format(tpl[0])))
