...
def lemmatize_TECs(dictionary):
	...
	for word in dictionary:
		...
		for TEC in dictionary[word]['TECs']:
			TECs.append(tuple([lemmatizer.lemmatize(TEC[0]).lower(), TEC[1], TEC[2], TEC[3]]))
...
def normalize_dictionary(dictionary):
	...
	for word in dictionary:
		if len(dictionary[word]['TECs']) > 2:
			...
			dictionary[word]['TECs'] = normalize_TECs(dictionary[word]['translations'], dictionary[word]['TECs'])
...
def normalize_TECs(translations, TECs):
	...
	for translation in translations:
		...
		most_similar = max(TEC_similarity_comparision, key=lambda k: TEC_similarity_comparision[k])
